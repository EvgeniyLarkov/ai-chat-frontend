...
const HeaderComponent = observer(() => {
	const { user, UiStorage } = useStore;
	const { t } = useTranslation();

	const handleLoginClick = action(() =>
		UiStorage.openModal(availableModals.login)
	);
						...
						{user.state === UserLoginStates.logined ? (
							<>
								<Typography.Text className="header-text">
									{t('header.greeting', { username: user.name })}
								</Typography.Text>
								<Button
									type="secondary"
									shape="round"
...
function IconRU() {
	return <Icon component={RuFlag} size={32} />;
}
...
function IconEN() {
	return <Icon component={EnFlag} size={32} />;
}
